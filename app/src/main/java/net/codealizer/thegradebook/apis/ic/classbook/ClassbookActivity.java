...
public class ClassbookActivity implements Serializable {
    ...
    public static Spanned getInfoString(Pair<String, ClassbookActivity> activityPair) {
        ...
        try {
            percent = new DecimalFormat(".##").format((Float.parseFloat(activity.score) / activity.totalPoints) * 100);
            letter = BasicGradeDetail.calculateGrade(percent);
        } catch (Exception ex) {
            percent = activity.score;
            letter = "";
        }

        if (activity.score != null && !percent.equals(activity.score)) {
            if (letter != null && letter.contains("A")) {
                return Html.fromHtml("You have earned an <strong>" + letter + " (" + percent + "%)</strong> for \"" + activity.name + "\" in <strong>" + WordUtils.capitalize(className.toLowerCase()) + "</strong>");
            } else {
                return Html.fromHtml("You have earned a <strong>" + letter + " (" + percent + "%)</strong> for \"" + activity.name + "\" in <strong>" + WordUtils.capitalize(className.toLowerCase()) + "</strong>");
            }
        } else {
            return Html.fromHtml("You have earned a <strong>" + percent + "</strong> for \"" + activity.name + "\" in <strong>" + WordUtils.capitalize(className.toLowerCase()) + "</strong>");

        }
    }

    public static class DateCompare implements Comparator {

        @Override
        public int compare(Object o, Object t1) {
            try {
                if (o instanceof ClassbookActivity && t1 instanceof ClassbookActivity) {
                    Date d1 = ((ClassbookActivity) o).getDueDateAsDate();
                    Date d2 = ((ClassbookActivity) t1).getDueDateAsDate();
